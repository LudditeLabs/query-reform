...
class HrefArea extends ImageMapArea {
    ...
    public void highlight(Graphics g, boolean on) {
        ...
        showStatus((on && anchor != null) ? "Go To " + anchor.toExternalForm() : null);
    }
...
class ClickArea extends ImageMapArea {
     ...
     */
    public void drag(int x, int y) {
        showStatus("Rectangle from " + ptstr(startx, starty) + " to " + ptstr(x, y) + " is " + (x - startx) + "x" + (y - starty));
    }
...
class RoundHrefButtonArea extends HrefButtonArea {
    ...
    public void makeImages() {
        upImage = parent.getHighlight(X, Y, W, H, new RoundButtonFilter(false, parent.hlpercent, border, W, H));
        downImage = parent.getHighlight(X, Y, W, H, new RoundButtonFilter(true, parent.hlpercent, border, W, H));
    }
}
...
class ButtonFilter extends RGBImageFilter {
    ...
    public void setColorModel(ColorModel model) {
        if (model instanceof IndexColorModel && true) {
            ...
            models[2] = null;
            if (pressed) {
                models[3] = filterIndexColorModel(icm, true, false, defpercent / 2);
            } else {
                models[3] = models[0];
            }
            models[4] = null;
                ...
                for (int i = 0; i < 7; i++) {
                    if (x2 > ranges[i] && x < ranges[i + 1]) {
                        ...
                        if (models[i] == null) {
                            ...
                            if (cx1 < cx2) {
                                consumer.setPixels(cx1, cy, cx2 - cx1, 1, models[i], pixels, off + (cx1 - x), scansize);
                            }
                        }
                    }
                }
...
class RoundButtonFilter extends ButtonFilter {
    ...
    public void buttonRanges(int y, int ranges[]) {
        ...
        if (y < border) {
            ...
            int xrel2 = (int) (Math.sqrt(Yrad2sq - yrel * yrel) * innerW / innerH);
